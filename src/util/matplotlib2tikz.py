...
def _draw_axes(data, obj):
    ...
    if isinstance(obj, mpl.axes.Subplot):  # @UndefinedVariable
...
def _handle_children(data, obj):
    ...
    content = []
    for child in obj.get_children():
        if (isinstance(child, mpl.axes.Axes)):  # @UndefinedVariable
            ...
            print('matplotlib2tikz: Don''t know how to handle object ''%s''.' %
                  type(child))
    # XXX: This is ugly
    if isinstance(obj, mpl.axes.Subplot) or isinstance(obj, mpl.figure.Figure):  # @UndefinedVariable
        for text in obj.texts:
