...
@author: wohlhart
    ...
    Disclaimer: the code is provided as is.
      Running this code might cause unpleasent and unexpected output ranging from killing your kitten
...
#         print("-------------------------------------------") 
    ...
    if inputMode < 3:
        assert len(allData) == 1, "input mode is {} but allData length {} > 1 ??".format(inputMode,len(allData)) 
        allData = allData[0]
...
def createNetwork(rng,cfg,train_set):
    ...
    # TODO: inputMode 5 = img / dpt separate into two streams, joined later in the network
...
def saveTrainResultsPickle(filename,descrNet,trainProgressData,cfg,trainerCfgParams):
...
def linemod_train_realandsynth_main(cfg):
    ...
    #############################################################################
    # CREATE NETWORK
    descrNet = createNetwork(rng,cfg,train_set)
    ...
    fn = resPath+startTimePidStr+"_b0_descrNet.pkl"
    saveTrainResultsPickle(fn,descrNet,trainProgressData,cfg,descrNetTrainer.cfgParams)
    ...
    for i in xrange(numBootstrappingRounds):
        ...
        # save intermediate descrNet
        fn = resPath+startTimePidStr+"_b{}_descrNet.pkl".format(i+1) 
        saveTrainResultsPickle(fn,descrNet,trainProgressData,cfg,descrNetTrainer.cfgParams)
    ...
    #################################################
    # save stuff
    fn = resPath+startTimePidStr+"_descrNet.pkl"
    saveTrainResultsPickle(fn,descrNet,trainProgressData,cfg,descrNetTrainer.cfgParams)
